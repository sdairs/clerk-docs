...
<Steps>
  ...
  When using the Sync Host feature, you might want to enable authentication methods on the web application that are unsupported in the extension. The Dashboard settings affect anything using the Clerk application, so those methods would show up in your Chrome Extension's sign-up and sign-in components. The following are options to hide unsupported methods.
